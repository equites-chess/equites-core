...
object GameStateUtil {
  def showFen(state: GameState): String = {
    val placement = BoardUtil.showFenPlacement(state.board)
  ...
  def showPgnMoveNumber(state: GameState): String =
    state.moveNumber.toString + state.color.fold(".", "...")
}
